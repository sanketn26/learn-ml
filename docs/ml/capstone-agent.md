...
??? note "Course details"
    ...
    **Runs on:** the framework venv (`make setup-frameworks`). No API key — the only model is an extractive stand-in (`capstone_agent.model`) that can only repeat the runbook it was handed, and it never routes.

---

## 🎯 What you will be able to do

- Route tickets with deterministic code, so no sentence a customer writes can open a write path
- Answer only from runbooks, and say "I don't know" when retrieval comes back empty
- Put every write behind a human approval that survives a restart — because both the checkpoint and the ledger are on disk
...
```
 ticket ─► triage (keyword firewall, no model)
             ...
             ├─ runbook hit ─────────────► docs           the model answers from retrieved text only
             ...
             └─ refund / credit ─► draft_credit ─║ interrupt ║─► issue_credit ─► ledger.credit(key)
                     ...
                     SqliteSaver + SQLite ledger (open_store) — survive a restart · golden tickets gate every change
```
...
| Checkpointed resume that survives a restart | `capstone_agent.storage.open_store` → `SqliteSaver` + a SQLite ledger, keyed by `thread_id` | LangGraph 3 |
| A model that can only say what retrieval gave it | `capstone_agent.model.extractive_model` | LangChain 4 |
...
There is no runbook for large exports — CW-1847 is an open incident. `retrieve` finds no runbook sharing two content words with the question, `triage` sends the ticket to `idk`, and the model is never called. An agent that answers the export question is inventing a fix for an incident engineering hasn't closed. Golden ticket g2 checks the route.

A miss has to be a real miss. "How do I fix a failed payment?" shares exactly one word — *failed* — with the password-reset runbook ("Five failed logins lock the account"). A retriever that counts that as a hit hands the model the wrong page, and the customer gets password instructions for a billing problem. So a hit needs `MIN_SHARED = 2` content words, and golden ticket g7 holds that line.

!!! warning "Watch out — a golden ticket that checks the doc id doesn't check the answer"

    With a canned fake model, every answer is the same sentence about API keys, whatever was retrieved. A test that only asserts `doc_ids == ["password-reset"]` passes while the customer reads the wrong instructions. The stand-in model here is *extractive*: it can only repeat the first sentence of the runbook it was handed. Tickets g1 and g8 assert what the answer says (`"Settings > API Keys"`, `"Forgot Password"`), and the test suite proves the gate fails an agent wired to a model that ignores its context.

## Money waits, and can wait across a restart

`interrupt_before=["issue_credit"]` stops the graph after `draft_credit` and checkpoints it. A human records `decision` with `update_state`; `invoke(None, config)` carries on.

Whether that pause survives a deploy depends entirely on *where* the checkpoint lives. `InMemorySaver` keeps it in the process: restart and the refund is gone — no pending approval, no record it was ever asked for. So is an in-memory ledger's memory of what it already paid. `open_store(directory)` puts both in SQLite files. The restart drill proves it: pause a refund, close every connection, open the same directory from a fresh agent, and `get_state(config).next` is still `("issue_credit",)`.

```
 process 1: invoke(REFUND) ─► paused at issue_credit ─► [deploy: process exits]
 process 2: open_store(dir) ─► get_state → next=("issue_credit",) ─► approve ─► credit lands ─► [dies]
 process 3: open_store(dir) ─► invoke(None) ─► issue_credit reruns ─► ledger: replayed=True ─► 2,900 cents, once
```

Golden tickets use `InMemorySaver` because each one is a fresh thread in one test process. Production uses `open_store`. If you only have the in-memory saver, say what it actually guarantees: a retry within one process, nothing more.
