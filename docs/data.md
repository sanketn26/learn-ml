...
everyone = load_customer_360(n=None)     # all 50k
```

`load_customer_360` is the Week 2 idea as a function: aggregate usage and events to **one row per user**, then left-join onto subscriptions.
